...
from ts_functions import HEIGHT, split_dataframe, sliding_window
...
max_iter = [500, 500, 1500, 2500, 5000]
...
plot_evaluation_results(trnY.data.numpy(), prd_trn, tstY.data.numpy(), prd_tst, f'length={best[0]} hidden={best[1]} episodes={best[2]}')
show()
savefig(f'images/LSTM/{file_tag}_LSTM_eval.png')

plot_forecasting_series(trainY, testY, prd_trn.values, prd_tst.values, f'LSTM PLOT length={best[0]} hidden={best[1]} episodes={best[2]}', x_label=index_col, y_label=target)
show()
savefig(f'images/LSTM/{file_tag}_LSTM_plot.png')
